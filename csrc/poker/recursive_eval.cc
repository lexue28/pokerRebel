      ...
      while (true) {
        ...
        for (size_t node = 0; node < tree.size(); ++node) {
          const auto& node_reaches =
              ...
              stats.reach_probabilities[Game::get_active_player(tree[node].state)][node];
...
// POKER VARIANT: Main evaluation program
int main(int argc, char* argv[]) {
  // POKER: Removed num_dice and num_faces (not needed)
  ...
  std::cout.setf(std::ios_base::fixed, std::ios_base::floatfield);
  {
    for (int i = 1; i < argc; i++) {
      ...
      // POKER: Removed --num_dice and --num_faces flags
  ...
  if (!net_path.empty()) {
    ...
    std::shared_ptr<IValueNet> net =
        net_path == "zero"
            ? poker::create_zero_net(game.num_hands(), false)
            : poker::create_torchscript_net(net_path);
    ...
    if (num_repeats > 0) {
      ...
      auto net_builder = [=]() {
        if (repeat_oracle_net) {
          ...
          return poker::create_oracle_value_predictor(game,
                                                           oracle_net_params);
        } else {
          return poker::create_torchscript_net(net_path, "cpu");
        }
      };
